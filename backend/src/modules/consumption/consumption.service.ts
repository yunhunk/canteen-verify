...
export class ConsumptionService {
   ...
   *
   * 两层防护：
   * 1. 公式注入：Excel/WPS 会把以 = + - @ 或制表符/回车开头的单元格当公式执行，
   *    员工姓名/门店名若被填成 `=cmd|'/c calc'!A1` 即可在打开导出文件的机器上执行命令。
   *    业界标准做法是在此类值前加一个单引号 `'`，Excel 会当纯文本显示且不回显该引号。
   *    注意必须**先**做这一步，再走引号包裹，否则 `"=1+1"` 仍会被部分解析器当公式。
   * 2. 分隔符转义：含逗号/引号/换行的字段用双引号包裹，内部引号翻倍，否则会串列。
   */
  private csvCell(v: unknown): string {
    if (v === null || v === undefined) return '';
    let s = String(v);
    // 公式注入中和：起始字符为 = + - @ 或制表符/回车时前置单引号。
    // 仅当值以单个 + / - 后跟数字时（如 -5、+3）不影响可读性，但为稳妥一律转义。
    if (/^[=+\-@\t\r]/.test(s)) {
      s = "'" + s;
    }
    if (/[",\r\n]/.test(s)) {
