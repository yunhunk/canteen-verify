...
import { platformRuleApi } from '@/api/rule';
...
async function load() {
  loading.value = true;
  try {
    list.value = (await platformRuleApi.list({ companyId: companyId.value || undefined })) || [];
  } catch {
    list.value = [];
  } finally {
    loading.value = false;
...
/** 跨天提示：end < start 表示跨天，比如夜宵 22:00-01:00 */
...
async function submit() {
  ...
  try {
    ...
    if (dialog.mode === 'create') {
      await platformRuleApi.create(payload);
      ElMessage.success('规则已创建');
    } else {
      await platformRuleApi.update(form.id, payload);
...
async function toggleStatus(row) {
  ...
  await confirmThen(
    ...
    async () => {
      await platformRuleApi.setStatus(row.id, next);
      await load();
    },
    `已${word}`,
  );
...
async function remove(row) {
  await confirmThen(
    ...
    async () => {
      await platformRuleApi.remove(row.id);
      await load();
    },
    '已删除',
    '删除规则',
